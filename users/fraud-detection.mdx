...
- **User metadata**: Update users with additional context like IP addresses, device fingerprints, and location data using the [Author API](/api-reference/author/update-an-author)
...
Every submission generates data points including:

- Content posting patterns and timing
- Login locations and device information
- Network characteristics and IP analysis
- Account metadata and verification status
...
- **API access**: Consume fraud scores and signals programmatically via the [Author API](/api-reference/author/get-an-author) for custom integrations
