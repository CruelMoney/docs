...
description: "Going over the basics of how to use content queues"
---
...
Content queues are a tool for reviewing and improving automated moderation. They are a way to review content that has been flagged by the model, take action on it, and optionally provide feedback to the model.
...
Content will then be added to the queue if it mathces the queue's configuration. By default only items that are [flagged](/content-moderation/flagging-thresholds) show up in the queue. See [how to configure queues](/content-queues/configuration#configuring-the-queue) to decide what content goes into a queue.
...
In the detail view you can also execute [actions](/content-queues/actions) for the item, or resolve the item.
...
4.  [By executeing an action that resolves the item](/content-queues/actions#resolving-an-item-when-action-is-executed)
    <br></br>
