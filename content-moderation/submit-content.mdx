...
const data = await fetch("https://api.moderationapi.com/v1/moderate", {
  method: "POST",
  headers: {
...
The [text moderation endpoint](/api-reference) allows for `conversationId` in the request body. _(Formerly `contextId`)_<br></br>

This could be the ID of a chatroom or a post thread.<br></br>

If you are using [review queues](/review-queues) the `conversationId` can also be used to filter the review queue to only show content from a specific context.

Enable [context awareness](#context-awareness) to improve the accuracy of your moderation using the conversationId.
...
Enable `Context awareness` in your project settings, and then include `authorId` and/or `conversationId` in the API request. This enables models to pull in previous messages for their analysis and increase their accuracy.

LLM models like AI agents can use the `conversationId` to see the previous messages in the same context, and `authorId` to see the previous messages from the same author.
...
[/moderate](/api-reference/moderate/analyze-text)
...
[/moderate](/api-reference/moderate/analyze-image)
...
[/moderate](/api-reference/moderate/analyze-object)
...
[/moderate](/api-reference/moderate/analyze-audio)
...
[/moderate](/api-reference/moderate/analyze-video)
