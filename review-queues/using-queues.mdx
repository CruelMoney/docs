...
Content is added to a queue when submitted to any of the [/moderate](/api-reference/moderate/analyze-text) endpoints. It shows up in a specific queue if it meets that queue’s configuration criteria. By default, only [flagged](/content-moderation/flagging-thresholds) items appear in the queue. Refer to [how to configure queues](/review-queues/configuration#configuring-the-queue) for more details.
