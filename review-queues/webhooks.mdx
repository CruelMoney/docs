...
Some hosting providers and security services may block webhook traffic. For example Cloudflare's Bot Fight mode.

### Check events log

If webhooks aren't working as expected, first check the [events log](https://moderationapi.com/app/moderation/events) in your dashboard to see detailed information about webhook delivery attempts and any failures.

### Configuration by provider

<Tabs>
  <Tab title="Cloudflare">
    **When Cloudflare blocks webhooks:**
    - Bot Fight Mode / Super Bot Fight Mode is enabled 
    - WAF Managed Rules active
    - Custom security rules

    **When using Super Bot Fight Mode (Pro plans and above):**
    The Skip action in the steps below will work to bypass Super Bot Fight Mode for webhook requests. Make sure to place it before the other rules.

    **When using Bot Fight Mode (Free plans):**
    Bot Fight Mode cannot be bypassed using Skip actions in WAF custom rules or Page Rules. Skip, Bypass, and Allow actions only apply to Ruleset Engine rules. To resolve this:
    - Consider upgrading to a Pro plan to use Super Bot Fight Mode (which can be bypassed)
    - Or contact support at [support@moderationapi.com](mailto:support@moderationapi.com) to get our IP addresses that need to be allowlisted in your Cloudflare configuration using [IP Access Rules](https://developers.cloudflare.com/waf/tools/ip-access-rules/create/)

    **Steps to allow webhooks:**
