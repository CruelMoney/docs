...
## Hosting provider and security integration

Some hosting providers and security services may block webhook traffic, preventing important notifications from reaching your server.

<Note>
  **When is this needed?** Only configure these settings if you experience webhook delivery issues or have aggressive security settings enabled.
</Note>

### Quick diagnosis

If webhooks aren't arriving, check for these common issues:

1. **Missing requests in logs** - No webhook requests reaching your endpoint
2. **Blocked requests** - Security logs showing blocked `ModAPI/` User-Agent
3. **Timeout errors** - Requests timing out before reaching your server

Our webhook identifier: **User-Agent starts with `ModAPI/`**
...
<Tabs>
  ...
  <Tab title="Other Providers">
    ...
    **General configuration steps:**
    ...
    4. Or add our IP addresses (contact support for list)
...
**Get help:**
Contact our support team at [support@moderationapi.com](mailto:support@moderationapi.com) for assistance with configuration and current IP addresses if needed.
