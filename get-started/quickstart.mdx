...
Use the API key to authenticate your requests to the API. You can submit content for analysis by sending a `POST` request to the `/api/v1/moderation/text` endpoint.

```javascript Text moderation example
const data = await fetch("https://moderationapi.com/api/v1/moderation/text", {
  method: "POST",
  headers: {
