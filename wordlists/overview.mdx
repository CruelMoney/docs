...
Each wordlist in a project will return its results under the `wordlists` field when you use the [moderate/text](/api-reference/moderate/analyze-text) endpoint. You can identify the wordlist by the `id` field or the `name`.
